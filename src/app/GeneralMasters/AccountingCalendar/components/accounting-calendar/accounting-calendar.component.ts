...
export class AccountingCalendarComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    const enterpriseId =
      this.localStorageMethods.loadEnterpriseData()?.id || '';
  ...
  private calculateStateHash(months: CalendarMonth[]): string {
    return months
      .map((m) => {
        ...
        const openDays = m.days.filter(
          (d) => d.isCurrentMonth && !d.isClosed,
        ).length;
  ...
  private initializeAvailableYears(): void {
    for (
      let year = CALENDAR_CONSTANTS.MIN_YEAR;
      year <= CALENDAR_CONSTANTS.MAX_YEAR;
      year++
    ) {
  ...
  onDateClick(day: CalendarDay): void {
    ...
    const isOpening = day.isClosed; // si estaba cerrado, vas a abrir
    const requiredPerms = isOpening ? ['A_C#OD'] : ['A_C#CD'];

    if (!this.authService.requireAnyPermission(requiredPerms)) return;
    ...
    this.messageService.add({
      ...
      detail: isOpening
        ? `${MESSAGE_CONSTANTS.SUCCESS.DATE_OPENED}: ${formattedDate}`
        : `${MESSAGE_CONSTANTS.SUCCESS.DATE_CLOSED}: ${formattedDate}`,
    });
  }
  ...
  onMonthStatusClick(month: CalendarMonth): void {
    const isOpening = month.status === MonthStatus.FULLY_CLOSED;

    const requiredPerms = isOpening ? ['A_C#OM'] : ['A_C#CM'];
  ...
  onToggleAll(): void {
    ...
    const requiredPerms = openAll ? ['A_C#OY'] : ['A_C#CY'];
  ...
  loadAllYearsState(): void {
    ...
    this.calendarService
      .getYearsWithOpenPeriods(enterpriseId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (yearsWithOpenPeriods) => {
          this.yearsWithOpenPeriods.clear();
          yearsWithOpenPeriods.forEach((year) =>
            this.yearsWithOpenPeriods.add(year),
          );
